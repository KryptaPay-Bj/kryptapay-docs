...
Which `recipient` fields are required depends on `network`:

| Network type | Required fields | Example |
| --- | --- | --- |
| Mobile money (e.g. `MTN_BJ`) | `msisdn` | `97000000`, `22997000000`, or `+22997000000` (local or international format) |
| `BANK_NGN` | `accountNumber` + `bankCode` | Nigerian NUBAN account number + bank code |
| Crypto (`ETH`, `POLYGON`, `TRON`) | `address` | Wallet address on that chain |
| Other bank rails (`BANK_XOF`, `BANK_XAF`, `BANK_GHS`, `BANK_CDF`, `BANK_EUR`, `BANK_USD`) | At least one recipient identifier | Settled manually by KryptaPay today |

<Warning>
  `recipient.bankName` and `recipient.bankId` are accepted but currently informational only — no payment provider reads them. Use `recipient.bankCode` for the bank identifier.
</Warning>
...
See [Testing → Failure Codes](/testing#failure-codes) for the complete list of failure codes and recommended actions.
