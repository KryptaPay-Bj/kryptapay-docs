...
<Info>
  `customer.msisdn` accepts either local or international format (e.g. `97000000` or `22997000000`/`+22997000000`). It's required whenever `network` is a mobile money network: every operator needs it to charge the customer. You can omit both `network` and `msisdn` if the customer will choose their payment method on a hosted checkout page.
</Info>
